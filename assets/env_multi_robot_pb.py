...
class Env(EnvBasePB):
    ...
    def reset(self,r1_loading_poses=None,r2_loading_poses=None):
        ...
        if self.args.gap_avoidance and self.args.insert_wall:
            ...
            p.removeBody(self.rectangle_id5)
        ...
        if self.args.num_robots ==4:
            ...
            if self.args.collision_likelihood_curr or self.args.rand_collision_likelihood:
                ...
                # print("robangle",self.robottogoal_angles)
            ...
            #     self.external_goals_states=[(8,0,0),(8,2,0)]
                for initial_pos,initial_goal_dist,robotgoal_angle in zip(self.initial_robots_pos,self.initial_goal_distances,self.robottogoal_angles):
                    # self.external_goal_state=self.find_position_B(robot_pos[1], initial_goal_dist, robotgoal_angle[1])
                    # if self.args.randomness==4:
                    #     self.external_goal_state=self.find_position_B(robot_pos[1], initial_goal_dist, robotgoal_angle[1])
                    # else:
                    self.external_goal_state=self.find_position_B(initial_pos[1], initial_goal_dist, robotgoal_angle[1])
                    self.external_goals_states.append(self.external_goal_state)
                ...
                for robot_pos,initial_goal_dist,robotgoal_angle in zip(self.external_robots_pos,self.initial_goal_distances,self.robottogoal_angles):
                    ...
                    self.external_goals_states_with_IDx.append((Robot,self.external_goal_state))
            # print("self.external_goal_state",self.external_goal_state,self.external_goals_states,robotgoal_angle[1])
            # print("self.external_goal_state_with_IDX",self.external_goals_states_with_IDx);exit()
        else:
            # synchoniser_value=np.random.uniform(1.5, 2.5)
            self.initial_goal_distances=[8,8]
            
            synchoniser_value=np.random.uniform(1.2, 2.5)
            if self.args.generalise:
                goal_offset_cross=np.random.uniform(7.0, 8.0)
            ...
            if self.args.collision_likelihood_curr:
                ...
                # self.robottogoal_angles=[(self.robots[0],self.robot_goal_synchroniser*7.5),(self.robots[1],self.robot_goal_synchroniser*-7.5)]
                # self.robottogoal_angles=[(self.robots[0],self.robot_goal_synchroniser*7.5),(self.robots[1],self.robot_goal_synchroniser*-7.5)]
                self.robottogoal_angles=[(self.robots[0],self.robot_goal_synchroniser*goal_offset),(self.robots[0],self.robot_goal_synchroniser*-goal_offset)]


            if self.args.randomness==0:
                self.external_robots_pos=[(self.robots[0],list(random_0_10)),(self.robots[0],[list(random_0_10)[0]+0,list(random_0_10)[1]+2,self.z_position])]
            elif self.args.randomness==1:
                # self.external_robots_pos=[(self.robots[0],list(random_0_10)),(self.robots[1],[list(random_0_10)[0]+0,list(random_0_10)[1]+self.robot_goal_synchroniser,self.z_position])]
                self.external_robots_pos=[(self.robots[0],list((random_0_10))),(self.robots[0],[list(random_0_10)[0]+0,list(random_0_10)[1]+self.robot_goal_synchroniser,self.z_position])]
            elif self.args.randomness==2:
                self.external_robots_pos=[(self.robots[0],list(random_0_10)),(self.robots[0],[list(random_0_10)[0]+np.random.choice([-1,0,-0,1]),list(random_0_10)[1]+self.robot_goal_synchroniser,self.z_position])]
            elif self.args.randomness==3:
                self.external_robots_pos=[(self.robots[0],list(random_0_10)),(self.robots[0],[list(random_0_10)[0]+np.random.choice([-1,1]),list(random_0_10)[1]+self.robot_goal_synchroniser,self.z_position])]
            # self.external_robots_pos=[(self.robots[0],[0,0,self.z_position]),(self.robots[1],[-2,-1.5,self.z_position])]
            # print("list(random_0_10)[1]-2",list(random_0_10)[1]-2)
            ...
            for robot_pos,initial_goal_dist,robotgoal_angle in zip(self.external_robots_pos,self.initial_goal_distances,self.robottogoal_angles):
                ...
                self.external_goals_states_with_IDx.append((Robot,self.external_goal_state))
            # print("RP",self.external_robots_pos,"GP",self.external_goals_states_with_IDx)
            # print(self.initial_goal_distances)
        ...
        #         # self.external_goal_state=self.find_position_B(robot_pos[1], initial_goal_dist, 10)
        ...
        if self.args.num_robots>1 and self.args.gap_avoidance:
            if self.args.gap_random:
                # print('Aw',self.All_Robot_ID[0].gap_width,self.All_Robot_ID[1].gap_width)
        ...
        # print('M_w',self.max_gap_among_all_robots_individual_gap_width)
        ...
        if self.args.gap_avoidance and self.args.insert_wall:
            ...
            obstacle_orn=np.random.uniform(-1,1)
            self.obstacle_box_orientations=[obstacle_orn]*self.args.num_robots
            perpendicular_orn = p.getQuaternionFromEuler((0, 0, p.getEulerFromQuaternion(Robot.gap_orn)[2] + (math.pi / 2)))
            perpendicular_box = p.getQuaternionFromEuler((0, 0, obstacle_orn + (math.pi / 2)))
            # perpendicular_orn = p.getQuaternionFromEuler((0, 0, np.random.randint(1,3)))
            # print(self.external_robots_pos[0][1][0])
            ...
            # print(Robot.external_robots_pos[0][1])
            
            if self.args.num_robots==2:
                robots_middle_point=((Robot.external_robots_pos[0][1][0] + Robot.external_robots_pos[1][1][0]) / 2,(Robot.external_robots_pos[0][1][1] + Robot.external_robots_pos[1][1][1]) / 2)
                random_x = random.uniform(robots_middle_point[0] + 1, robots_middle_point[0] + 4)  # In front of the robot on the x-axis
                random_y = random.uniform(robots_middle_point[1] - 1.5, robots_middle_point[1] + 1.5)  # Within a 3m range on the y-axis
            elif self.args.num_robots==1:
                random_x = random.uniform(Robot.external_robots_pos[0][1][0] + 1, Robot.external_robots_pos[0][1][0] + 4)  # In front of the robot on the x-axis
                random_y = random.uniform(Robot.external_robots_pos[0][1][1] - 1.5, Robot.external_robots_pos[0][1][1] + 1.5)  # Within a 3m range on the y-axis
            self.raondom_pos = [random_x,random_y]
    ...
    def visualize_maps(self,global_map_list, local_heightmaps, local_heightmap_positions, robot_positions,robot_orientations, goal_positions,obstalce_positions,gap_walls1_centre,gap_walls2_centre,side_walls1_centre,side_walls2_centre):
        ...
        #             x_index=obstacles_x_index[i]
        #             y_index=obstacles_y_index[i]
        ...
        # print(self.obstacle_box_orientations)
        ...
        if self.args.gap_avoidance and self.args.obstacle:
            for global_map in global_map_list:
                for i in range(len(self.Obstacles_pos)):
                    ...
                    x_index = obstacles_x_index[i]
                    y_index = obstacles_y_index[i]
                    orientation = obstacles_orientation[i]  # Assuming you have obstacle orientation in radians
        ...
        #             # Use the rotated corners to update the global map (draw a rotated rectangle)
        #             for corner in rotated_corners:
        #                 x_rot, y_rot = corner
        #                 x_rot = int(np.clip(x_rot, 0, self.global_num_rows - 1))
        #                 y_rot = int(np.clip(y_rot, 0, self.global_num_cols - 1))

        #                 global_map[x_rot, y_rot] = 1.0  # Mark the rotated obstacle position on the map
        ...
        #Other Robot appearning in occupancy map
        ...
        if self.args.map_show_function:
    ...
    def create_obstacle(self,ID,pos,wall_length,wall_width,wall_height,orientation):

        # Calculate the center and half extents of the rectangle
        #half_extents = [(corners[2][i] - corners[0][i])/2 for i in range(3)]
        
        center = [pos[0], pos[1], wall_height/2]
        # print("c",center)
        # center=[center[0],center[1]+1,center[2]]
        # print("ac",center)

        half_extents=[wall_length/2, wall_width/2, wall_height/2]
