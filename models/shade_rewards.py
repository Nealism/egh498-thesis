...
def plot_performance(epochs_list, means_list, stds_list, labels, colors):
    ...
    for epochs, means, stds, label, color in zip(epochs_list, means_list, stds_list, labels, colors):
        # Plot mean accuracy
        plt.plot(epochs, means, marker='o', linestyle='-', linewidth=2, label=label, color=color)
    ...
    plt.axvspan(0, 63, facecolor='none', edgecolor='magenta', hatch='/', linewidth=2)  # Adjust linewidth to 0 for hatching only
    plt.axvline(x=63, color='magenta', linestyle='-', linewidth=5)  # Separator line

    plt.axvspan(63, 107, facecolor='none', edgecolor='purple', hatch='\\\\', linewidth=2)  # Adjust linewidth to 0 for hatching only
    plt.axvline(x=107, color='purple', linestyle='-', linewidth=5)  # Separator line

    # Creating legend entries for hatch areas
    hatch_patch1 = Patch(facecolor='none', edgecolor='magenta', hatch='///', label='MA_Bootstrap Cur Phase')
    hatch_patch2 = Patch(facecolor='none', edgecolor='purple', hatch='\\\\', label='Gap Cur Phase')
...
def main():
    ...
    directories = ['/home/kom018/titan_results_data_tensorboard/reward/Bootstrap_GAP/', '/home/kom018/titan_results_data_tensorboard/reward/GAP/', '/home/kom018/titan_results_data_tensorboard/reward/End_to_End/']
