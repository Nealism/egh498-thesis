...
"""
class Terrain():
    def __init__(self, terr_arr, img_path, img_name):
    ...
    def load_terr_img(self):
        ...
        """ 
        path = self.img_path + self.img_name
        cv2.imwrite(path, self.terr_arr)
        img = cv2.imread(path)
        return img
    ...
    def display_box(self, pos, n, m, colour=(0,0,0), thickness=1):
        """
        Displays the terrain image with a bounding box of size nxm 
        centred at the given position

        NOTE: this is used to display the robot's section of the map (ie: the 
        PNG subsection it sees) and is mostly for debugging purposes

        Params:
            pos - centre of bounding box
