...
NEW_BRANCH_BASE_CHANCE = 0.1
NEW_BRANCH_CHANCE_FACTOR = 1.05
# NEW_BRANCH_CHANCE_FACTOR = 1.5
...
class geom():
    ...
    def generate_xml(self):
        colour = "0.1 " + str(np.clip(self.branch_id / MAX_BRANCHES, 0.1, 1.0)) + " 0.1 1.0"
...
class tree_segment():
    ...
    def __init__(self, radius, half_height, damping, stiffness, parent, branch_id, pos_id, pos, rot, spacing, chance_of_new_branch, prev_world_coords, root = False):
        radius = np.clip(radius, 0.001, 1.0)
        half_height = np.clip(half_height, 0.001, 1.0)
        self.radius = radius
        ...
        # print(pos, rot, self.branch_id, self.pos_id)

        # print(self.branch_id, self.pos)       
        if self.branch_id == 0:
            if self.pos_id == 0:
                # print(pos, rot)
                self.radius = 0.025
                self.half_height = 0.6
            # if self.pos_id == 1:
                # spacing = 0.2
            else:
                pos[2] = self.pos[2] = self.half_height
                self.radius = 0.02
                self.half_height = 0.05
        ...
        # print("creating geom", branch_id, pos_id, radius, half_height)
        self.geom = geom(branch_id, pos_id, rot, self.radius, self.half_height)
    ...
    def next_segment(self):
        ...
        # print(next_pos_id, self.branch_id, self.pos_id)
        # print(self.radius, self.half_height)
        #     seg = tree_segment(0.05, 1.2, self.damping, self.stiffness, self, 
        #                         self.branch_id, next_pos_id, self.rot_spacing, self.rot, self.spacing, 
        #                         NEW_BRANCH_BASE_CHANCE, self.world_coords)
        # else:
        seg = tree_segment(self.radius, self.half_height, self.damping, self.stiffness, self, 
                            self.branch_id, next_pos_id, self.rot_spacing, self.rot, self.spacing, 
                            NEW_BRANCH_BASE_CHANCE, self.world_coords)
        return seg

def generate_tree(base_radius, base_half_height, base_damping, base_stiffness, spacing):
    ...
    # print("apparentall ythis is the height of the trunk ", base_half_height)
    ...
    # root_seg = tree_segment(0.05, 1.2, base_damping, base_stiffness, 
    # root_seg = tree_segment(0.02, 1.2, base_damping, base_stiffness, 
                #   None, 0, 0, root_pos, TRUNK_ROTATION, spacing, NEW_BRANCH_BASE_CHANCE, root_pos, root=True)
    
    # print(base_radius, base_half_height)
    # first_branch = tree_segment(base_radius, base_half_height, base_damping, base_stiffness, 
    #               None, 0, 0, root_pos, TRUNK_ROTATION, spacing, NEW_BRANCH_BASE_CHANCE, root_pos)
    trunk = True
    #Generate the rest of the tree
    frontier_segments = [root_seg]
    # frontier_segments = [first_branch]
    all_segs = []
    branch_ends = []
    ...
    while frontier_segments:
        ...
        if((np.random.uniform(0, 1) < new_seg.chance_of_new_branch and 
            ...
            # branch_rot = [0,0,0.2,0.4]
            # branch_rot = [0,0,0.4,0.2]
            # branch_rot = [0,0,0,1.0]
            branch_rot = branch_rot / np.linalg.norm(branch_rot)
            # print(new_seg.pos_id)

            # if new_seg.pos_id == 1:
            #     new_seg.pos[2] = 1.0
            # # if True:
            #     new_branch_seg = tree_segment(base_radius * BRANCH_SIZE_LOSS_FACTOR, 
            #                         base_half_height * BRANCH_SIZE_LOSS_FACTOR, 
            ...
            #                         new_seg.spacing * BRANCH_SIZE_LOSS_FACTOR, NEW_BRANCH_BASE_CHANCE, seg.world_coords, root=True)
