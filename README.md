...
- Mount scratch workspace, this is where results and data are typically saved (there isn't much room on the cluster for saving results). This is for temporary storage only. Need to use the datastore if need to save things for longer periods.
    ...
    - if you don't have sshfs installed, you need to install it: `sudo apt update && sudo apt install sshfs`
...
- Copy code to HPC: <br/> 
- When you are copying code using rsync you need to be in the folder where the "rsync_exclude.txt" exists, i.e. you need to be in the `behaviour_rl` repository.
