...
def dqn(env, ac_kwargs=dict(), seed=0, 
    ...
    #print("obs_size",env.observation_space.shape[0])
    ...
    if use_perception:
        ...
        policy_net = DQN(env.observation_space.shape[0], env.action_space.n, **ac_kwargs)
        
        #policy_net = DQN(n_observations, n_actions).to(device)
        target_net = DQN(env.observation_space.shape[0], env.action_space.n, **ac_kwargs)#.to(device)
    ...
    sync_params(target_net)

    # Count variables
    #var_counts = tuple(core.count_vars(module) for module in [ac.pi, ac.v])
    var_counts = tuple(core.count_vars(module) for module in [policy_net, target_net])
    ...
    # Set up model saving
    logger.setup_pytorch_saver(policy_net)
    ...
    for epoch in range(epochs):
        ...
        if (epoch % save_freq == 0) or (epoch == epochs-1):
            if proc_id() == 0:
                print("Saving model")
                torch.save(policy_net, PATH + "model.pt")
        ...
        if proc_id() == 0:
            ...
            #writer.add_scalar("Stds", np.mean(policy_net.std.data.numpy()), epoch)
        ...
        #logger.log_tabular('Std', np.mean(policy_net.std.data.numpy()))
