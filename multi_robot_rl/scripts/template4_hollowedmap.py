    ...
    while not rospy.is_shutdown():
        ...
        if R1.robot_pose is not None and R1.robot_vel is not None and R1.goal_pos is not None and R1.im is not None and R3.robot_pose is not None and R3.robot_vel is not None and R3.goal_pos is not None and R3.im is not None:
            ...
            # Result array with shape (2, 1, 80, 80)
                converted_images = np.zeros((total_robots, 1, 80, 80), dtype=np.uint8)
                
                # Only process the last two heightmaps
                for index in range(total_robots):
                    heightmap_image = heightmap_images[index + total_robots]
            ...
            occupancy_map_locals = convert_heightmap_images(joint_map)
            ...
            # listing=np.array(listing).reshape(total_robots,self.im_size)
            Hollowed_Occupancy_maps=np.array(Hollowed_Occupancy_maps).reshape([total_robots]+self.im_size)
            ...
            b=z_net(torch.as_tensor(Hollowed_Occupancy_maps, dtype=torch.float32))
            # print(a[0],"br1",b)
        ...
        if R3.current_sim_time.to_sec()>20:
            ...
            output_video = '/refarm/src/multi_robot_rl/scripts/plots/r1_image/r1_map.mp4'
            ...
            # for idx, img in enumerate(R3.robot3_images):
            #     plt.imshow(img)
            #     plt.axis('off')  # Hide axes
            #     plt.title(f"Image {idx+1}")
            ...
            # Path to save the output video
            output_video = '/refarm/src/multi_robot_rl/scripts/plots/r2_image/r2_map.mp4'
