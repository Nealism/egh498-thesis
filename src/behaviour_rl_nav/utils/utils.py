...
def display_images(im1, im2=None, im3=None, lag=1):
    ...
    if im2 is None:
        ...
        if len(im2.shape) < 4:
            im2 = im2.reshape([-1] + list(im2.shape))
        if len(im3.shape) < 4:
            im3 = im3.reshape([-1] + list(im3.shape))
        for i1, i2, i3 in zip(im1, im2, im3):
            i1 = cv2.resize(i1,dsize=(int(i1.shape[1] * 6), int(i1.shape[0] * 6)))
            i2 = cv2.resize(i2,dsize=(int(i2.shape[1] * 6), int(i2.shape[0] * 6)))
            i3 = cv2.resize(i3,dsize=(int(i3.shape[1] * 6), int(i3.shape[0] * 6)))
