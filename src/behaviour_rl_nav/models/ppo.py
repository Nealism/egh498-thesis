...
def ppo(env_fn, ac_kwargs=dict(), seed=0, 
        ...
        target_kl=0.01, logger_kwargs=dict(), save_freq=10, PATH=None, writer=None):
    """
    ...
    def update(epoch):
        ...
        for i in range(train_pi_iters):
            ...
            # loss.backward()
            # mpi_avg_grads(ac.pi + ac.v)    # average grads across MPI processes
            # optimizer.step()
    ...
    for epoch in range(epochs):
        ...
        # Save model
        print((epoch % save_freq == 0) or (epoch == epochs-1))
        if (epoch % save_freq == 0) or (epoch == epochs-1):
            torch.save(ac, PATH + "pyt_save/model.pt")
            print("running test")
            pb_save_state = p.saveState()
            test_success = run_test(env, PATH + "pyt_save/model.pt")
            writer.add_scalar("SuccessTest", test_success, epoch)
            p.loadState(pb_save_state)
            # logger.save_state({'env': env}, None)

        # Perform PPO update!
        update(epoch)

        # Perform logging and tensorboard stuff
        ...
        if proc_id() == 0:
            writer.add_scalar("Rews", np.mean(rewbuffer), epoch)
            writer.add_scalar("Lens", np.mean(lenbuffer), epoch)
        ...
        # Turn off these additional things for now
...
def run_test(env, model):
    ...
    ac.eval()
    cur = env.args.cur
    apply_disturbances = env.args.apply_disturbances
    env.args.cur = False
    env.args.disturbances = False
    ob = env.reset()
    done = False
    while not done:
        ac, _, _ = ac.step(ob, stochastic=False)
        ob, rew, done, _ = env.step(ac)
    success = env.body_xyz[0] > 15
    env.args.cur = cur
    env.args.apply_disturbances = apply_disturbances
    successes = MPI.COMM_WORLD.allgather(success)
    print("Test success: ", successes)
