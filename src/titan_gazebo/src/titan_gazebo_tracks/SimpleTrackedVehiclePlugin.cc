...
#include "opende/joints/contact.h"
                                      ...
                                      sdf::ElementPtr _sdf)
{
  ...
  this->physicsStepSize = _model->GetWorld()->Physics()->GetMaxStepSize();
}
...
void SimpleTrackedVehiclePlugin::Init()
{
  ...
  this->currentTrackVelocity[Tracks::LEFT] = 0.0;
  this->currentTrackVelocity[Tracks::RIGHT] = 0.0;

  this->trackAccelPerLoop = this->GetTrackAcceleration() * this->physicsStepSize;
}
...
void SimpleTrackedVehiclePlugin::SetTrackVelocityImpl(double _left,
                                                      double _right)
{
  this->desiredTrackVelocity[Tracks::LEFT] = _left;
  this->desiredTrackVelocity[Tracks::RIGHT] = _right;
}
    ...
    const common::UpdateInfo &/*_unused*/)
{
  ...
  /////////////////////////////////////////////
  if (std::abs(this->currentTrackVelocity[Tracks::LEFT] - this->desiredTrackVelocity[Tracks::LEFT]) < this->trackAccelPerLoop)
  {
    this->currentTrackVelocity[Tracks::LEFT] = this->desiredTrackVelocity[Tracks::LEFT];
  }
  else if (this->currentTrackVelocity[Tracks::LEFT] > this->desiredTrackVelocity[Tracks::LEFT])
  {
    this->currentTrackVelocity[Tracks::LEFT] = this->currentTrackVelocity[Tracks::LEFT] - this->trackAccelPerLoop;
  }
  else if (this->currentTrackVelocity[Tracks::LEFT] < this->desiredTrackVelocity[Tracks::LEFT])
  {
    this->currentTrackVelocity[Tracks::LEFT] = this->currentTrackVelocity[Tracks::LEFT] + this->trackAccelPerLoop;
  ...
  if (std::abs(this->currentTrackVelocity[Tracks::RIGHT] - this->desiredTrackVelocity[Tracks::RIGHT]) < this->trackAccelPerLoop)
  {
    this->currentTrackVelocity[Tracks::RIGHT] = this->desiredTrackVelocity[Tracks::RIGHT];
  }
  else if (this->currentTrackVelocity[Tracks::RIGHT] > this->desiredTrackVelocity[Tracks::RIGHT])
  {
    this->currentTrackVelocity[Tracks::RIGHT] = this->currentTrackVelocity[Tracks::RIGHT] - this->trackAccelPerLoop;
  }
  else if (this->currentTrackVelocity[Tracks::RIGHT] < this->desiredTrackVelocity[Tracks::RIGHT])
  {
    this->currentTrackVelocity[Tracks::RIGHT] = this->currentTrackVelocity[Tracks::RIGHT] + this->trackAccelPerLoop;
  ...
  // gzerr << "Track Vels: des [" << this->desiredTrackVelocity[Tracks::LEFT] << "," << this->desiredTrackVelocity[Tracks::RIGHT]
  //       << "], curr [" << this->currentTrackVelocity[Tracks::LEFT] << "," << this->currentTrackVelocity[Tracks::RIGHT] << "]" << std::endl;

  const auto leftBeltSpeed = -this->currentTrackVelocity[Tracks::LEFT];
  const auto rightBeltSpeed = -this->currentTrackVelocity[Tracks::RIGHT];
  ...
  for (auto contact : contacts)
  {
    ...
    dBodyID left = dynamic_cast<physics::ODELink&>(
      *this->tracks[Tracks::LEFT]).GetODEId();
    dBodyID right = dynamic_cast<physics::ODELink& >(
...
SimpleTrackedVehiclePlugin::ContactIterator::operator++()
{
  ...
  for (; this->jointIndex < static_cast<size_t>(dBodyGetNumJoints(this->body));
         this->jointIndex++)
  {
    const dJointID joint_id = dBodyGetJoint(this->body,
                                            static_cast<int>(this->jointIndex));

    // only interested in contact joints
    if (dJointGetType(joint_id) != dJointTypeContact)
      continue;
    ...
    dContact* odeContact = &(static_cast<dxJointContact *>(joint_id)->contact);
