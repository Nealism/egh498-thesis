...
def run(args): 
    ...
    # for p in np.arange(2, 2, 0):
    # for p in 2:
    # for y in range(1.2,2.5):
    # for q in np.arange(0.6, 0.9, 0.3):
    X1, Y1, U1, V1 = [], [], [], []
    X2, Y2, U2, V2 = [], [], [], []
    for g in np.arange(0.53,0.54, 0.01):
        ...
        # y=
            ...
            obs = env.reset()
            # obs = [np.array([obs[0][0],obs[0][1],-0.2,0,0,0]),np.array([obs[1][0],obs[1][1],-0.75,-0.01,0,0])]
            ...
            # r2_clipped_linear_vel_command=np.clip(action[1][0], -0.0, 0.0)
            # r2_clipped_linear_vel_command=np.clip(action[1][0], np.random.choice([-00,-0.6]), np.random.choice([00,0.7]))
...
#---------------------------------------------------------------------------------------------------------
                ...
                # # if format(r1_buffer_linear_acon[i] * 0.5
                ...
                print("VELS",r2_buffer_linear_action[i],r2_buffer_angular_action[i])
                width2 = 0.1
                alpha2 = 0.5
    ...
    # if (abs(dx2) < 1e-6 and abs(dy2) < 1e-6) or (r2_buffer_linear_action[i]<0.5 and r2_buffer_linear_action[i]>-0.5):
    # if (r2_buffer_linear_action[i]<0.5 and r2_buffer_linear_action[i]>-0.5):
    #     # continue
    ...
    #         # is_reverse = m2 < 0
    #         # arrow_color2 = 'red' if is_reverse else 'blue'
    #         arrow_color2 = 'black'

    #         # ax2.arrow(k2,l2,m2,n2, width=0.01, head_width=width2, color=arrow_color2, alpha=alpha2)
    #         ax2.plot(k2, l2, marker='s', color='black', markersize=8, alpha=alpha2)
    # else:
    #     for k2,l2,m2,n2 in zip(X2, Y2, U2, V2):
    #         is_reverse = m2 < 0
    #         arrow_color2 = 'red' if is_reverse else 'blue'
    #         # arrow_color2 = 'red' if m2 < 0 else 'blue' if m2 > 0 else 'black'
    ...
    #             # ax2.arrow(x, y, dx2, dy2, width=0.01, head_width=width2, color=arrow_color, alpha=alpha2)
            ...
            #     # else:
            #     if  r2_buffer_linear_action[i] == 0.75:
            #         is_reverse = m2 < 0
            #         arrow_color2 = 'red' if is_reverse else 'blue'
            #         ax2.arrow(k2, l2, m2, n2, width=0.01, head_width=width2, color=arrow_color2, alpha=alpha2)
            #     elif -0.5 < r2_buffer_linear_action[i] < 0.5:
            #         ax2.plot(k2, l2, marker='s', color='black', markersize=8, alpha=alpha2)
            
            # for k2, l2, m2, n2 in zip(X2, Y2, U2, V2):
            #     if p==1 and q ==1:
            #         r2_buffer_linear_action[i] = 0.0
            #         m2 = 0.0
            #     # else:
            #     is_reverse = m2 < 0
            #     arrow_color2 = 'red' if is_reverse else 'blue'
            #     ax2.arrow(k2, l2, m2, n2, width=0.01, head_width=width2, color=arrow_color2, alpha=alpha2)
            #     if r2_buffer_linear_action[i] == 0.0 or r2_buffer_linear_action[i] == -0.0:
            #         ax2.plot(k2, l2, marker='s', color='black', markersize=8, alpha=alpha2)
            for k2, l2, m2, n2 in zip(X2, Y2, U2, V2):
                # if -0.5 < r2_buffer_linear_action[i] < 0.5:
                #     ax2.plot(k2, l2, marker='s', color='black', markersize=8, alpha=alpha2)
                # else:
                # if p==1 and q ==1:
                #     r2_buffer_linear_action[3] = 0.0
                #     m2 = 0.0

                if round(r2_buffer_linear_action[i], 1) == 0.0:
                    print("BINGO_________________")
                    m2 = 0
                    # n2=0
                if m2 == 0:
                    print("BINGO1_________________")
                    arrow_color2 = 'black'
                    widtharrow=0.2
                    hd=0.1
                    hl=0.05
                ...
                ax2.arrow(k2, l2, m2, n2, width=widtharrow, head_width=hd, head_length=hl, color=arrow_color2, alpha=alpha2)
...
# print(f"t={buffer_time[i]:.2f}s, lin_vel2={r2_buffer_linear_action[i]:.10f}, format2={format(r2_buffer_linear_action[i], '.2f')}")
    ...
    # else:
        ...
        # Add to plot
            ...
            # ax2.plot([Goal2_x_list[0]-4, Goal2_x_list[0]-4],
            ...
            # ax2.set_xlim(mid_x - 5, mid_x + 5)
            # ax2.set_ylim(mid_y , mid_y )
            ax2.set_xlim(mid_x - 8, mid_x + 5)
            # ax2.set_xlim(mid_x - 5, mid_x + 1)
            ax2.set_ylim(mid_y - 1.2, mid_y + 2)
            # ax2.set_ylim(mid_y - 1, mid_y + 0.3)
            # ax2.set_aspect("equal", adjustable="datalim")
            # ax22.set_aspect("equal", adjustable="datalim")

            # ax2.set_aspect('auto')  # optional: or use 'equal' if you want uniform scaling
            # ax2.set_aspect('equal')  # optional: or use 'equal' if you want uniform scaling
            ax2.set_xlabel("X position (m)", fontsize=20)
            ax2.set_ylabel("Y position (m)", fontsize=20)
