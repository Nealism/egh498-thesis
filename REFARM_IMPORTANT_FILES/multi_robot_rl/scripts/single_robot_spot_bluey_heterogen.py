...
class RLRobot():
    ...
    def __init__(self, cmd_topic, topic1_robotpos, topic3_robotvel, topic5_goalpos, occupancy_topic, model_full_fc,cnn_to_vector, cnn):
        self.action_pub = rospy.Publisher(cmd_topic, Twist)
    ...
    def step_R1(self):
        ...
        if self.robot_pose is not None and self.robot_vel is not None and self.goal_pos is not None and self.im is not None:
            ...
            clipped_lateral_vel_command=np.clip(self.actions[0][1].detach().numpy(), -0.3, 0.3)
            clipped_angular_vel_command=np.clip(self.actions[0][2].detach().numpy(), -0.75, 0.75)

            #change from tensor to float just , not np array
            self.actions[0]=torch.tensor([clipped_linear_vel_command,clipped_lateral_vel_command,clipped_angular_vel_command])
            
            msg.linear.x = self.actions[0][0]
            msg.linear.y = self.actions[0][1]
            msg.angular.z = self.actions[0][2] 
...
if __name__ == "__main__":
    ...
    spot_output_layer = torch.load("/refarm/src/multi_robot_rl/scripts/JIT_models/Jit_model_Heterogeneous/spot_output_13.jit")
    ...
    R1 = RLRobot("/spot/cmd_vel_joy","/r2/slam/odom/high/pose","/r2/slam/odom/high/velocity",goal_R1, "/r2/costmap_local/occupancy_grid", spot_output_layer, feature_layers, z_net)
    # print()
