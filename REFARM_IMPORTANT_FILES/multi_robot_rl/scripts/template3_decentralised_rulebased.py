...
class RLRobot():
    ...
    def step_R1(self):
        ...
        robot1_stop_duration=np.random.uniform(1, 2)
        if self.robot_pose is not None and self.robot_vel is not None and self.goal_pos is not None and self.im is not None:
            ...
            if R1.current_sim_time.to_sec() <2:
                self.actions=torch.tensor([[-0.3,0]])
            elif R1.current_sim_time.to_sec() <2 + robot1_stop_duration:
                self.actions_r3=torch.tensor([[0,0]])
            else: 
    ...
    def step_R3(self):
        msg = TwistStamped() # create a message
        robot2_stop_duration=np.random.uniform(8, 9)
        
        if self.robot_pose is not None and self.robot_vel is not None and self.goal_pos is not None and self.im is not None:
            self.observations = self.get_observation()
            self.im_ocupancy=self.get_occupancy_image()
            if R3.current_sim_time.to_sec() <2:
                self.actions_r3=torch.tensor([[-0.3,0]])

            elif R3.current_sim_time.to_sec() <2 + robot2_stop_duration:
                self.actions_r3=torch.tensor([[0,0]])
            else: 
...
if __name__ == "__main__":
    ...
    while not rospy.is_shutdown():
        ...
        # print(R1.current_sim_time.to_sec(),R3.current_sim_time.to_sec())
